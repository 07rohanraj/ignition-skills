---
name: perspective-named-query
description: Creating, editing, and using Ignition Named Queries (SQL, parameters, resource.json, dataset/json/scalar returns). Use when creating or modifying Named Queries for Query bindings or system.db.runNamedQuery.
---
...
# Ignition Named Query Parameter Types (Ignition 8.3)

The `sqlType` field in a Named Query `resource.json` uses Ignition's internal SQL type IDs, **not** Java/JDBC `java.sql.Types` constants.
...
Do **not** use JDBC `java.sql.Types` values (for example `93` for `TIMESTAMP`) in `resource.json`.
...
Using JDBC type values will cause errors such as:

```
java.lang.IllegalArgumentException:
Enum does not contain a value for '93'
```
